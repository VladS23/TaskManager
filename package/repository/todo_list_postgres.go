...
func (r *TodoListPostgres) Create(userId int, list todo.TodoList) (int, error) {
	...
	var id int
	createListQuery := fmt.Sprintf("INSERT INTO %s (title, descriptions) VALUES ($1, $2) RETURNING id", todoListTable)
...
func (r *TodoListPostgres) Update(userId int, listId int, input todo.UpdateListInput) error {
	...
	query := fmt.Sprintf("UPDATE %s tl SET %s FROM %s ul WHERE tl.id =ul.list_id AND ul.list_id=$%d AND ul.user_id=$%d", todoListTable, setQuery, userListTable, argId, argId+1)
